...
+---------------+-------------------+ */
-- Type your code below:
# first part:-  Number of movies released each year.
SELECT Year, COUNT(id) as number_of_movies
FROM movie
GROUP BY year
ORDER BY year;

#second part :- Number of movies released each month.
...
SELECT YEAR, COUNT(id) AS MOVIES_Produced_InYear
FROM movie
...
SELECT Year, Genre,  COUNT(movie_id) AS Popular_Genre
FROM genre 
INNER JOIN movie 
ON movie_id = id
WHERE year = 2019
...
WITH One_Genre AS
(
	SELECT movie_id, COUNT(genre) AS Movies_Single_Genre
	FROM genre
	GROUP BY movie_id
	HAVING Movies_Single_Genre=1
)

SELECT COUNT(movie_id) AS Movies_Single_Genre
FROM One_Genre;
...
ON Gen.movie_id = Ra.movie_id
INNER JOIN movie AS Mo
ON Mo.id = Gen.movie_id

WHERE  MONTH(date_published)=3  AND year=2017 AND Mo.country='USA' AND  Ra.total_votes>1000   
GROUP BY Gen.genre
ORDER BY movie_count DESC;
...
(
	...
	INNER JOIN ratings AS r 
	ON r.movie_id = g.movie_id, top_3_genre
	WHERE g.genre in (top_3_genre.genre) AND avg_rating>8
	GROUP BY director_name
	ORDER BY movie_count DESC
)

SELECT * FROM top_3_director
...
SELECT 	name AS  actress_name, 
		...
		ROUND(avg_rating,2) AS actress_avg_rating,
...
date_difference AS
(
	 SELECT *, DATEDIFF(next_movie_date, date_published) AS dif
	 FROM movie_date_info
 ),
 
 avg_inter_days AS
 (
	 SELECT name_id, AVG(dif) AS avg_inter_movie_days
